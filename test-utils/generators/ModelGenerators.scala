...
trait ModelGenerators {
  ...
  def genRateBandForRegime(alcoholRegime: AlcoholRegime): Gen[RateBand] =
    for {
      ...
      rate           <- Gen.chooseNum(0.00, 99999.99).map(BigDecimal(_).setScale(1, BigDecimal.RoundingMode.UP))
  ...
  def genVolumeAndRateByTaxTypeRateBand(rateBand: RateBand): Arbitrary[VolumeAndRateByTaxType] = Arbitrary {
    for {
      ...
      sprDutyRate <- Gen.chooseNum(0.00, 99999.99).map(BigDecimal(_).setScale(1, BigDecimal.RoundingMode.UP))
    } yield VolumeAndRateByTaxType(
      rateBand.taxTypeCode,
      totalLitres.setScale(2, BigDecimal.RoundingMode.UP),
