...
object WhiskySummary  {
  ...
  def row(answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(WhiskyPage).map {
      answer =>

      val value = HtmlFormat.escape(answer.ScotchWhisky.toString()).toString + "<br/>" + HtmlFormat.escape(answer.IrishWhiskey.toString()).toString
