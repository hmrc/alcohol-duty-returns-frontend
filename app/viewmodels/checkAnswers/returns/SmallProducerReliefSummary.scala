...
object SmallProducerReliefSummary {
  def summaryList(regime: AlcoholRegime, userAnswers: UserAnswers)(implicit messages: Messages): Option[SummaryList] =
    userAnswers.getByKey(DoYouHaveMultipleSPRDutyRatesPage, regime) map { hasMultipleSPREntries =>
      if (hasMultipleSPREntries) {
        createSummaryList(
          ...
          call = controllers.returns.routes.MultipleSPRListController.onPageLoad(regime).url
        )
      } else {
        createSummaryList(
          regime,
          ...
          call = controllers.returns.routes.TellUsAboutSingleSPRRateController.onPageLoad(CheckMode, regime).url
        )
