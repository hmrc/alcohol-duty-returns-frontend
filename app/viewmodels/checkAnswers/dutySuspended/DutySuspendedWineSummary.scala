...
object DutySuspendedWineSummary  {
  ...
  def row(answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(DutySuspendedWinePage).map {
      answer =>

      val value = HtmlFormat.escape(answer.totalWine.toString()).toString + "<br/>" + HtmlFormat.escape(answer.pureAlcoholInWine.toString()).toString
