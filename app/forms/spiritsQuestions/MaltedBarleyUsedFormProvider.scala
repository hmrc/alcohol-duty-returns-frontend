...
class MaltedBarleyUsedFormProvider @Inject() extends Mappings {

  def apply(): Form[BigDecimal] =
    Form(
      "malted-barley-used-input" -> bigDecimal(
        2,
        "maltedBarleyUsed.error.required",
        "maltedBarleyUsed.error.nonNumeric",
        "maltedBarleyUsed.error.twoDecimalPlaces"
      )
        .verifying(minimumValue(BigDecimal(0.01), "maltedBarleyUsed.error.minimumRequired"))
