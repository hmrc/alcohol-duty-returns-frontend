...
import models.dutySuspended.DutySuspendedQuantities
...
class DutySuspendedQuantitiesFormProvider @Inject() extends Mappings {
  def apply(regime: AlcoholRegime): Form[DutySuspendedQuantities] = Form(
    mapping(
      totalLitresInsideUkMapping(regime),
      pureAlcoholInsideUkMappings,
      totalLitresOutsideUkMapping(regime),
      pureAlcoholOutsideUkMapping,
      totalLitresReceivedMapping(regime),
      pureAlcoholReceivedMapping
    )(DutySuspendedQuantities.apply)(DutySuspendedQuantities.unapply)
  )

  private def pureAlcoholReceivedMapping =
  ...
  private def totalLitresReceivedMapping(regime: AlcoholRegime) =
  ...
  private def pureAlcoholOutsideUkMapping =
  ...
  private def totalLitresOutsideUkMapping(regime: AlcoholRegime) =
  ...
  private def pureAlcoholInsideUkMappings =
  ...
  private def totalLitresInsideUkMapping(regime: AlcoholRegime) =
