...
class CheckDSDOldJourneyToggleAction @Inject() (appConfig: FrontendAppConfig)(implicit
  ...
  override protected def refine[A](request: DataRequest[A]): Future[Either[Result, DataRequest[A]]] =
    if (!appConfig.dutySuspendedNewJourneyEnabled) {
      ...
      logger.warn("New duty suspended journey switched on, cannot access old journey")
