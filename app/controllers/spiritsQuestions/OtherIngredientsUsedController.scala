...
class OtherIngredientsUsedController @Inject() (
  ...
  formProvider: OtherIngredientsUsedFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: OtherIngredientsUsedView
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] =
    (identify andThen getData andThen requireData andThen checkSpiritsRegime andThen checkSpiritsAndIngredientsToggle) {
      implicit request =>
        val preparedForm = request.userAnswers.get(OtherIngredientsUsedPage) match {
          case None        => form
      ...
      .async { implicit request =>
        form
          ...
          .fold(
            ...
            value =>
              for {
                updatedAnswers <- Future.fromTry(request.userAnswers.set(OtherIngredientsUsedPage, value))
                _              <- cacheConnector.set(updatedAnswers)
              } yield Redirect(navigator.nextPage(OtherIngredientsUsedPage, mode, updatedAnswers))
          )
      }
}
