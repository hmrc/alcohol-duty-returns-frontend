...
class AlcoholDutyTaskListHelperSpec extends SpecBase with ScalaCheckPropertyChecks {
  ...
  val taskListService             = new TaskListService(returnTaskListCreator)
  ...
  "AlcoholDutyTaskListHelper" - {
    "must return an incomplete task list" in {
      ...
      val result           =
        taskListService.getTaskList(emptyUserAnswers, validUntil, periodKeyMar)(
    ...
    "must return a completed task list" in {
      ...
      val result           =
        taskListService.getTaskList(userAnswers, validUntil, periodKeyMar)(
          messages(application)
        )
      val validUntilString = validUntil.toLocalDateString()
    ...
    "must return the quarterly spirits task only in Mar, Jun, Sep and Dec" in {
      ...
      forAll(periodKeyGen) { case periodKey =>
        val result =
          taskListService.getTaskList(emptyUserAnswers, validUntil, periodKey)(
            messages(application)
          )
