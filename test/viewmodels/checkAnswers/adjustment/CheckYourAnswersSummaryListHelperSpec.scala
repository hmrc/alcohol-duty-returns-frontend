...
class CheckYourAnswersSummaryListHelperSpec extends SpecBase {
  "CheckYourAnswersSummaryListHelper must return all rows when adjustment type summary, volume summary, and duty due summary can be fetched" in new SetUp(true, true, true) {
    checkYourAnswersSummaryListHelper.currentAdjustmentEntrySummaryList(adjustmentEntry) mustBe Some(SummaryListViewModel(rows = Seq(row6, row4, row3, row5, row1, row2, row7, row8)))
  }

  "CheckYourAnswersSummaryListHelper must return no rows when adjustment type summary can't be fetched" in new SetUp(false, true, true) {
    checkYourAnswersSummaryListHelper.currentAdjustmentEntrySummaryList(adjustmentEntry) mustBe None
  }

  "CheckYourAnswersSummaryListHelper must return all rows when volume summary can't be fetched" in new SetUp(true, false, true) {
    checkYourAnswersSummaryListHelper.currentAdjustmentEntrySummaryList(adjustmentEntry) mustBe None
  }

  "CheckYourAnswersSummaryListHelper must return all rows when duty due summary can't be fetched" in new SetUp(true, true, false) {
    checkYourAnswersSummaryListHelper.currentAdjustmentEntrySummaryList(adjustmentEntry) mustBe None
  }

  class SetUp(canFetchAdjustmentTypeSummary: Boolean, canFetchAdjustmentVolumeSummary: Boolean, canFetchDutyDueSummary: Boolean) {
