...
class DeclareSmallProducerReliefDutyRateControllerSpec extends SpecBase with MockitoSugar {
  ...
  val validAnswer = 0.01

  lazy val declareSmallProducerReliefDutyRateRoute = routes.DeclareSmallProducerReliefDutyRateController.onPageLoad(NormalMode).url
  ...
  "DeclareSmallProducerReliefDutyRate Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(DeclareSmallProducerReliefDutyRatePage, validAnswer).success.value
