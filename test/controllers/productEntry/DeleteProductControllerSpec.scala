...
import models.productEntry.DeleteProduct
...
class DeleteProductControllerSpec extends SpecBase with MockitoSugar {
  ...
  "DeleteProduct Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(DeleteProductPage, DeleteProduct.values.head).success.value
      ...
      running(application) {
        val request =
          FakeRequest(POST, deleteProductRoute)
            .withFormUrlEncodedBody(("deleteProduct-value", DeleteProduct.values.head.toString))
    ...
    "redirect to Journey Recovery for a POST if no existing data is found" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, deleteProductRoute)
            .withFormUrlEncodedBody(("value", DeleteProduct.values.head.toString))
