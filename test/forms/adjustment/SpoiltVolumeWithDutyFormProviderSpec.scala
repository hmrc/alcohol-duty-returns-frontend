...
class SpoiltVolumeWithDutyFormProviderSpec extends StringFieldBehaviours with ModelGenerators with SpecBase {
  ...
  ".volumes" - {
    ...
    "fail to bind when invalid values are provided" in {
      ...
      form.bind(data).errors must contain allElementsOf Seq(
        FormError("volumes_totalLitresVolume", "spoiltVolumeWithDuty.error.invalid.totalLitresVolume", Seq("", "")),
        FormError("volumes_pureAlcoholVolume", "spoiltVolumeWithDuty.error.invalid.pureAlcoholVolume", Seq("", "")),
    ...
    "fail to bind when values below minimum are provided" in {
      ...
      form.bind(data).errors must contain allElementsOf Seq(
        FormError(
          "volumes_totalLitresVolume",
          "spoiltVolumeWithDuty.error.minimumValue.totalLitresVolume",
          Seq("", "")
        ),
        FormError(
          "volumes_pureAlcoholVolume",
          "spoiltVolumeWithDuty.error.minimumValue.pureAlcoholVolume",
          Seq("", "")
        ),
    ...
    "fail to bind when values exceed maximum are provided" in {
      ...
      form.bind(data).errors must contain allElementsOf Seq(
        FormError(
          "volumes_totalLitresVolume",
          "spoiltVolumeWithDuty.error.maximumValue.totalLitresVolume",
          Seq("", "")
        ),
        FormError(
          "volumes_pureAlcoholVolume",
          "spoiltVolumeWithDuty.error.maximumValue.pureAlcoholVolume",
          Seq("", "")
        ),
    ...
    "fail to bind when pure alcohol volume is higher than total litres value" in {
      ...
      form.bind(data).errors must contain allElementsOf Seq(
        FormError("volumes_pureAlcoholVolume", "spoiltVolumeWithDuty.error.lessThanExpected", Seq("", ""))
      )
    }
    ...
    "fail to bind when pure alcohol volume is empty, total litres value exceeds maximum and duty is invalid" in {
      ...
      form.bind(data).errors must contain allElementsOf Seq(
        ...
        FormError(
          "volumes_pureAlcoholVolume",
          Seq("spoiltVolumeWithDuty.error.noValue.pureAlcoholVolume"),
          Seq("", "")
        ),
        FormError("volumes_duty", Seq("spoiltVolumeWithDuty.error.invalid.duty"), Seq("", ""))
      )
    }
    ...
    "fail to bind with decimal places error when pure alcohol, total litres and duty have more than expected decimals and are also out of range" in {
      ...
      form.bind(data).errors must contain allElementsOf Seq(
        ...
        FormError("volumes_duty", Seq("spoiltVolumeWithDuty.error.decimalPlaces.duty"), Seq("", ""))
      )
    }
  }
}
