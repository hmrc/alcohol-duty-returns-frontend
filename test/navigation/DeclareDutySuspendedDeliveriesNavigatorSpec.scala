    ...
    "in Normal mode" - {
      ...
      Seq(
        ...
        s"must go from the Duty suspended beer page to the correct page when the next regime is ${regime.entryName}" in {
          navigator.nextPage(
            ...
            NormalMode,
            emptyUserAnswers.copy(regimes = AlcoholRegimes(Set(regime, Beer)))
          ) mustBe expectedPage()
        }
      }
      ...
      Seq(
        ...
        s"must go from the Duty suspended cider page to the correct page when the next regime is ${regime.entryName}" in {
          navigator.nextPage(
            ...
            NormalMode,
            emptyUserAnswers.copy(regimes = AlcoholRegimes(Set(regime, Cider)))
          ) mustBe expectedPage()
        }
      }

      "must go from the Duty suspended cider page to the check your answers page if no other regime was found" in {
        navigator.nextPage(
          pages.dutySuspended.DutySuspendedCiderPage,
          NormalMode,
      ...
      Seq(
        ...
        s"must go from the Duty suspended wine page to the correct page when the next regime is ${regime.entryName}" in {
          navigator.nextPage(
            ...
            NormalMode,
            emptyUserAnswers.copy(regimes = AlcoholRegimes(Set(regime, Wine)))
          ) mustBe expectedPage()
        }
      }

      "must go from the Duty suspended wine page to the check your answers page if no other regime was found" in {
        navigator.nextPage(
          pages.dutySuspended.DutySuspendedWinePage,
          NormalMode,
          userAnswersWithWine
      ...
      "must go from the Duty suspended spirits page to the correct page when the next regime is OtherFermentedProduct" in {
        navigator.nextPage(
          ...
          NormalMode,
          emptyUserAnswers.copy(regimes = AlcoholRegimes(Set(Spirits, OtherFermentedProduct)))
      ...
      s"must go from the Duty suspended deliveries other fermented products page to CYA page" in {
        navigator.nextPage(
